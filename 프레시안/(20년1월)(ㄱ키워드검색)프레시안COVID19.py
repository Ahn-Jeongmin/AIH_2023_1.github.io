...
base_url = "https://www.pressian.com/pages/search?sort=1&search=%EC%BD%94%EB%A1%9C%EB%82%98&startdate=2020.01.19&enddate=2020.01.31&page={}"  #객관적인 자료 확보를 위해 아무 검색 키워드 없이 정치 면 크롤링 (정치일반)
html_urls = []
for page in range(1,60):
    url = base_url.format(page)
    html_urls.append(url)

base_url = "https://www.pressian.com/pages/search?sort=1&search=%EC%BD%94%EB%A1%9C%EB%82%98&startdate=2020.02.01&enddate=2020.02.28&page={}"  #객관적인 자료 확보를 위해 아무 검색 키워드 없이 정치 면 크롤링 (정치일반)
html_urls = []
for page in range(1,62):
    url = base_url.format(page)
    html_urls.append(url)

base_url = "https://www.pressian.com/pages/search?sort=1&search=%EC%BD%94%EB%A1%9C%EB%82%98&startdate=2020.03.01&enddate=2020.03.19&page={}"  #객관적인 자료 확보를 위해 아무 검색 키워드 없이 정치 면 크롤링 (정치일반)
html_urls = []
...
plt.xlabel("단어")
plt.ylabel("빈도수")
plt.title("프레시안 20년 1월 코로나19 키워드 검색 관련 기사 단어 빈도수 분석")
plt.show()

# 결과 출력
print("프레시안 20년 1월 코로나19 키워드 검색 관련 기사 중 가장 많이 등장한 단어 50개:")
